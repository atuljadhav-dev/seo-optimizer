...
export const analyzePageLinks = async (
	...
	try {
		...
		if (!url) {
			errorResponse(
				res,
				'A valid URL is required to analyze backlinks.',
				400,
			);
			return;
		}
		...
		try {
			...
			errorResponse(res, 'Invalid URL format provided.', 400);
			return;
		}
		...
		if (!response.ok) {
			errorResponse(
				res,
				`Failed to fetch the page. Status: ${response.status}`,
				response.status,
			);
			return;
		}
		...
		successResponse(
			res,
			'Backlink analysis completed successfully.',
			report,
		);
	} catch (error: any) {
		errorResponse(
			res,
			'Link profile scanning system error.',
			500,
			error.message,
		);
	}
