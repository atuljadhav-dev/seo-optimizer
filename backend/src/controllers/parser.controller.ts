...
export const analyzeExternalUrl = async (
	...
	try {
		...
		if (!url) {
			errorResponse(
				res,
				'A valid URL is required for SEO analysis.',
				400,
			);
			return;
		}
		...
		try {
			...
			errorResponse(
				res,
				'Invalid URL formatting. Include http:// or https://',
				400,
			);
			return;
		}
		...
		if (!response.ok) {
			errorResponse(
				res,
				`Failed to reach target. Status: ${response.status}`,
				422,
			);
			return;
		}
		...
		successResponse(
			res,
			'SEO metrics parsed successfully.',
			{ url: targetUrl.href, data: metrics },
			200,
		);
	} catch (error: any) {
		errorResponse(
			res,
			'Scraping engine failed to process the destination site.',
			500,
		);
	}
};
